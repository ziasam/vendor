...
func init() {
	android.RegisterModuleType("kangos_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = kangosExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = kangosExpandVariables(ctx, dep_file)
	...
	cmd := kangosExpandVariables(ctx, String(g.properties.Cmd))
