...
function setup_vendor() {
    ...
    export KANGOS_ROOT="$3"
    if [ ! -d "$KANGOS_ROOT" ]; then
        echo "\$KANGOS_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$KANGOS_ROOT/$OUTDIR" ]; then
        mkdir -p "$KANGOS_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$KANGOS_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$KANGOS_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$KANGOS_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$KANGOS_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) $YEAR The KangOS\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-$YEAR The KangOS\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR The KangOS\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 The KangOS\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The KangOS\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR The KangOS\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The KangOS\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The KangOS\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$KANGOS_ROOT"/prebuilts/tools-kangos/common/smali/baksmali.jar
        export SMALIJAR="$KANGOS_ROOT"/prebuilts/tools-kangos/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$KANGOS_ROOT"/prebuilts/tools-kangos/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$KANGOS_ROOT"/prebuilts/tools-kangos/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$KANGOS_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$KANGOS_TARGET" >/dev/null; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$KANGOS_ROOT"/vendor/lineage/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$KANGOS_ROOT"/"$OUTDIR"/radio
